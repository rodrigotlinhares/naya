...
int main(void)
{
	...
	tracker.Initialize8DOF(size_template_x,
						   size_template_y,
						   n_active_pixels,
						   n_bins,
						   size_bins,
	...
	tracker.ComputeActive8DOF(&Template, 0, active_pixels_r, active_pixels_g, active_pixels_b);
	...
	while(key != 'q' && key != 'Q')
    {
		...
		// Run naya
		tracker.Run8DOF(&ICur, 0, &Template, 0, parameters, active_pixels_r, active_pixels_g, active_pixels_b);
	
		// Display
        key = tracker.Display8DOF(&ICur, 1);
