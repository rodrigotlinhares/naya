...
int main(void)
{
    ...
    LoadParameters("../source_pub/parameters.yml",
                   &size_template_x,
				   &size_template_y,
				   &n_ctrl_pts_x,
				   &n_ctrl_pts_y,
    ...
    // Temporary!
    // I haven't implemented active pixel selection for the TPS warp! Sorry ;)
	...
	tracker.InitializeTPS(size_template_x,
		 				  size_template_y,
		 				  n_active_pixels,
		 				  n_ctrl_pts_x,
						  n_ctrl_pts_y,
	...
	parameters = (float*)malloc(2*n_ctrl_pts_x*n_ctrl_pts_y*sizeof(float));

	tracker.InitTPSParameters(parameters, coords); 
	...
	while(key != 'q' && key != 'Q')
	{
		...
		// Run Naya
		tracker.RunTPS(&ICur, 0, &Template, 0, parameters, 0, 0, 0);
	
		// Display
        key = tracker.DisplayTPS(&ICur, 1);
