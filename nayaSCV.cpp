...
void naya::Initialize6DOF(int size_template_x, 
                          int size_template_y, 
                          int n_bins, 
                          int size_bins,
                          int n_max_iters,
                          ...
                          int interp)
{
    ...
    dummy_mapx.create(size_template_y, size_template_x, CV_32FC1);
    dummy_mapy.create(size_template_y, size_template_x, CV_32FC1);
    Hess.create(6, 6, CV_32FC1);
    delta.create(6, 1, CV_32FC1);
    Mask.create(size_template_y, size_template_x, CV_8UC1);
    Mask.setTo(255);
    ...
    {
        dif.create(size_template_x*size_template_y, 1, CV_32FC1);
        SD.create(size_template_x*size_template_y, 6, CV_32FC1);
        Template_comp.create(size_template_y, size_template_x, CV_8UC1);
    ...
    {
        dif.create(size_template_x*size_template_y*3, 1, CV_32FC1);
        SD.create(size_template_x*size_template_y*3, 6, CV_32FC1);
        Template_comp.create(size_template_y, size_template_x, CV_8UC3);
							...
							int interp)
{
	...
	Hess.create(2*total_n_ctrl_pts, 2*total_n_ctrl_pts, CV_32FC1);
...
void naya::InitializeCompositional6DOF(int size_template_x, 
                                       int size_template_y, 
                                       int n_bins, 
                                       int size_bins,
                                       int n_max_iters,
                  ...
                  float *parameters)
{
    ...
    {        
        ...
        // Mounts Jacobian
        isgrayscale ? MountJacobian6DOFGray() : MountJacobian6DOFColor();
...
void naya::Warp6DOF()
{
    ...
    for(int i=-offy;i<offy;i++)
    {
        for(int j=-offx;j<offx;j++)
        {
            dummy_mapx.at<float>(i+offy,j+offx) = parameters[0]*(float)j + parameters[2]*(float)i + parameters[4];
            dummy_mapy.at<float>(i+offy,j+offx) = parameters[1]*(float)j + parameters[3]*(float)i + parameters[5];
        }
    }
...
{	
	...
	for(int i1=0; i1<size_template_y; i1++)
	{
		for(int j1=0; j1<size_template_x; j1++)
		{
			...
			{
				// gradients
				float sum_gradx = gradx.at<float>(i1,j1) ;
				float sum_grady = grady.at<float>(i1,j1) ;
	...
	for(int i1=0; i1<size_template_y; i1++)
	{
		for(int j1=0; j1<size_template_x; j1++)
		{
			if(Mask.at<uchar>(i1,j1) != 0)
			{
				...
				// Jacobian
				SD.at<float>(3*k, 0) = gradx.ptr<float>(i1)[3*j1] ;
				SD.at<float>(3*k, 1) = grady.ptr<float>(i1)[3*j1] ;
				SD.at<float>(3*k+1, 0) = gradx.ptr<float>(i1)[3*j1+1] ;
				SD.at<float>(3*k+1, 1) = grady.ptr<float>(i1)[3*j1+1] ;
				SD.at<float>(3*k+2, 0) = gradx.ptr<float>(i1)[3*j1+2] ;
				SD.at<float>(3*k+2, 1) = grady.ptr<float>(i1)[3*j1+2] ;
...
void naya::MountJacobian6DOFGray()
{
    int offx = cvCeil((double)size_template_x/2);
    int offy = cvCeil((double)size_template_y/2);
...
//            if(Mask.at<uchar>(row,col) != 0)
            {
                // gradients
                float sum_gradx = gradx.at<float>(row,col);
                float sum_grady = grady.at<float>(row,col);

                // img difference
                dif.at<float>(k, 0) = (float)(current_warp.at<uchar>(row,col)
                                      - Template_comp.at<uchar>(row,col));

                // gradient
                SD.at<float>(k, 0) = col * sum_gradx;
                SD.at<float>(k, 1) = col * sum_grady;
                SD.at<float>(k, 2) = row * sum_gradx;
                SD.at<float>(k, 3) = row * sum_grady;
                SD.at<float>(k, 4) = sum_gradx;
                SD.at<float>(k, 5) = sum_grady;
...
void naya::MountJacobian6DOFColor()
{
    int offx = cvCeil((double)size_template_x/2);
    int offy = cvCeil((double)size_template_y/2);
...
//            if(1)
            ...
            {
                // gradients (red)
                float gradx_r = gradx.ptr<float>(row)[3*col];
                float grady_r = grady.ptr<float>(row)[3*col];

                // gradients (green)
                float gradx_g = gradx.ptr<float>(row)[3*col+1];
                float grady_g = grady.ptr<float>(row)[3*col+1];

                // gradients (blue)
                float gradx_b = gradx.ptr<float>(row)[3*col+2];
                float grady_b = grady.ptr<float>(row)[3*col+2];

                // img difference (red)
                dif.at<float>(3*k, 0) = (float)(current_warp.ptr<uchar>(row)[3*col]
                                        - Template_comp.ptr<uchar>(row)[3*col]);

                // img difference (green)
                dif.at<float>(3*k+1, 0) = (float)(current_warp.ptr<uchar>(row)[3*col+1]
                                          - Template_comp.ptr<uchar>(row)[3*col+1]);

                // img difference (blue)
                dif.at<float>(3*k+2, 0) = (float)(current_warp.ptr<uchar>(row)[3*col+2]
                                          - Template_comp.ptr<uchar>(row)[3*col+2]);

                // gradient (red)
                SD.at<float>(3*k, 0) = col * gradx_r;
                SD.at<float>(3*k, 1) = col * grady_r;
                SD.at<float>(3*k, 2) = row * gradx_r;
                SD.at<float>(3*k, 3) = row * grady_r;
                SD.at<float>(3*k, 4) = gradx_r;
                SD.at<float>(3*k, 5) = grady_r;

                // gradient (green)
                SD.at<float>(3*k+1, 0) = col * gradx_g;
                SD.at<float>(3*k+1, 1) = col * grady_g;
                SD.at<float>(3*k+1, 2) = row * gradx_g;
                SD.at<float>(3*k+1, 3) = row * grady_g;
                SD.at<float>(3*k+1, 4) = gradx_g;
                SD.at<float>(3*k+1, 5) = grady_g;

                // gradient (blue)
                SD.at<float>(3*k+2, 0) = col * gradx_b;
                SD.at<float>(3*k+2, 1) = col * grady_b;
                SD.at<float>(3*k+2, 2) = row * gradx_b;
                SD.at<float>(3*k+2, 3) = row * grady_b;
                SD.at<float>(3*k+2, 4) = gradx_b;
                SD.at<float>(3*k+2, 5) = grady_b;
            ...
            else
            {
                // img difference
                dif.at<float>(k, 0) = 0;
                dif.at<float>(k+1, 0) = 0;
                dif.at<float>(k+2, 0) = 0;
...
{	
	...
	// ESM update
	delta = ((SD.t()*SD).inv(CV_LU)*(SD.t()*dif));
...
{	
    ...
    parameters[2] -= delta.at<float>(2, 0); sum += fabs(delta.at<float>(2, 0));
    parameters[3] -= delta.at<float>(3, 0); sum += fabs(delta.at<float>(3, 0));
    parameters[4] -= delta.at<float>(4, 0); sum += fabs(delta.at<float>(4, 0));
    parameters[5] -= delta.at<float>(5, 0); sum += fabs(delta.at<float>(5, 0));
...
{	
	...
	// Update
	float sum = 0;
    parameters[0] = update_auxH.at<float>(0, 0); sum += fabs(delta.at<float>(0, 0));
    parameters[1] = update_auxH.at<float>(0, 1); sum += fabs(delta.at<float>(1, 0));
    parameters[2] = update_auxH.at<float>(0, 2); sum += fabs(delta.at<float>(2, 0));
    parameters[3] = update_auxH.at<float>(1, 0); sum += fabs(delta.at<float>(3, 0));
    parameters[4] = update_auxH.at<float>(1, 1); sum += fabs(delta.at<float>(4, 0));
    parameters[5] = update_auxH.at<float>(1, 2); sum += fabs(delta.at<float>(5, 0));
    parameters[6] = update_auxH.at<float>(2, 0); sum += fabs(delta.at<float>(6, 0));
    parameters[7] = update_auxH.at<float>(2, 1); sum += fabs(delta.at<float>(7, 0));
...
{	
	...
	if(lambda != 0)
	{
		...
		for(int i=0; i<total_n_ctrl_pts; i++)
		{
            ctrl_pts_x_w.ptr<float>(i)[0] -= delta.at<float>(i, 0); sum += fabs(delta.at<float>(i, 0));
            ctrl_pts_y_w.ptr<float>(i)[0] -= delta.at<float>(i+total_n_ctrl_pts, 0); sum += fabs(delta.at<float>(i+total_n_ctrl_pts, 0));
		}
	}
	else
	{
...
int	naya::Display2DOF(cv::Mat *ICur, int delay)
{
	cv::line(*ICur,
...
int	naya::Display4DOF(cv::Mat *ICur, int delay)
{
	cv::line(*ICur,
...
int	naya::Display6DOF(cv::Mat *ICur, int delay)
{
    int off_x = size_template_x / 2;
    int off_y = size_template_y / 2;
    cv::Point tl(-off_x * parameters[0] + -off_y * parameters[2] + parameters[4],
                 -off_x * parameters[1] + -off_y * parameters[3] + parameters[5]);
    cv::Point bl(-off_x * parameters[0] + off_y * parameters[2] + parameters[4],
                 -off_x * parameters[1] + off_y * parameters[3] + parameters[5]);
    cv::Point tr(off_x * parameters[0] + -off_y * parameters[2] + parameters[4],
                 off_x * parameters[1] + -off_y * parameters[3] + parameters[5]);
    cv::Point br(off_x * parameters[0] + off_y * parameters[2] + parameters[4],
                 off_x * parameters[1] + off_y * parameters[3] + parameters[5]);

    cv::line(*ICur, tl, bl, CV_RGB(255, 255, 255), 2, 8, 0);
    cv::line(*ICur, bl, br, CV_RGB(255, 255, 255), 2, 8, 0);
    cv::line(*ICur, br, tr, CV_RGB(255, 255, 255), 2, 8, 0);
    cv::line(*ICur, tr, tl, CV_RGB(255, 255, 255), 2, 8, 0);
...
int	naya::Display8DOF(cv::Mat *ICur, int delay)
{
...
int	naya::DisplayTPS(cv::Mat *ICur, int delay)
{
